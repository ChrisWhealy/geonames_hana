...
cds.connect(connectionObj)
   ...
   .then(countryList => {
      console.log(`Fetching GeoName and Alternate Name files for ${countryList.length} countries`)

      Promise
        .all(countryList.map(el => loader.geonamesHandler(el)
                                         .then((resolve, reject) => loader.altNamesHandler(el))))
        .then(() => {
